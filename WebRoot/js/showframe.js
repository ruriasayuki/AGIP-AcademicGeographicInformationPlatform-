...
function renderItem(params, api) {
	...
	EchartSeries = [
        ...
        },
        {
            ...
            type: 'effectScatter',
            coordinateSystem: 'bmap',
            data: convertData(datae.sort(function (a, b) {
    ...
    var citys = {
            ...
            '浙江': Math.random() * 70,
            '江苏': Math.random() * 70,
			'乌里雅苏台': Math.random() * 70,
			'直隶': Math.random() * 70,
			'奉天': Math.random() * 70
        }

        $.getJSON('./data/new_qing_prov.json', function(geojson) {
            ...
            var data = dataSet.get({
                filter: function (item) {//数据字段和geojson的name匹配 这里特殊化了一下 因为清代省份图用了两个字段 分别表示拼音和中文的省份名
                    if (!citys[item.name_ch]) {
                        return false;
                    }

                    item.count = citys[item.name_ch];
                    return true;
                }
            });
