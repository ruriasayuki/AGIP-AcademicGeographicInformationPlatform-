...
//字符串 分段数,
//字符串类型 暂时只有linear)
function yukiColorMaper(min,max,minColor,maxColor,num,style)
{
	...
	if(style="linear")
		...
		while(nowend<max)
		{
            var nowColor = {r:parseInt(valueColor.r),
            ...
            valueColor =  {r:valueColor.r+rstep,
                g:valueColor.g+gstep,
            b:valueColor.b+bstep};
            nowstart=nowend;
            nowend = nowend+step;
		}
			ColorMaps.push({start: min,end: 10,value: minColor})
		}
    return ColorMaps;
}
